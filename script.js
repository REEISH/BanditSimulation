...
function initState() {
  ...
  shuffleArray(array);
  for (let i = 0; i < 5; i++) {
    STOCKS[i].trueMean = Distribution[array[i]].mean;
    STOCKS[i].trueStd = Distribution[array[i]].std;
  }
  state = {
    round: 0,
    cash: STARTING_CASH,
...
function playRound(stockIdx) {
  ...
  // IMP LINE
  if (SessionActive) sendState(state);
}
...
async function login() {
  ...
  if (entered === PASSWORD) {
    try {
      ...
      //console.log("Login error:", data.error); // Debugging line
      if (data.error) {
        // Debugging line
